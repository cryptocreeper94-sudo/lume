...
const codebase = [
    ...
    { module: 'Healer (L2)', file: 'src/runtime/healer.js', loc: 260, desc: 'Layer 2: Self-Healing. Retry with exponential/linear/fixed backoff + jitter. Circuit breaker pattern (CLOSED→OPEN→HALF-OPEN). AI model fallback chains, plus true AST-mutating @auto-rewrite.' },
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
                        ...
                        <div className="bento-card" style={{ padding: 24 }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                                <div><div style={{ fontSize: 28, fontWeight: 800, color: 'var(--accent-glow)', fontFamily: 'var(--font-mono)' }}>{serverMeta?.tests || 'CI-verified'}</div><div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Tests Passing</div></div>
                                ...
                                <div><div style={{ fontSize: 28, fontWeight: 800, color: 'var(--accent-glow)', fontFamily: 'var(--font-mono)' }}>{serverMeta?.version || 'v0.9.0'}</div><div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{serverHealth ? `Uptime: ${Math.floor(serverHealth.uptime / 60)}m` : 'Version'}</div></div>
                            </div>
                        </div>
